...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        True
        """
        # for every row in the gird
        for r in range(len(self._grid)):
            for c in range(len(self._grid[r])):
                # for every column in the grid (thus, checking every item)
                if self._grid[r][c] == EMPTY_CELL \
                        and len(self._symbol_set
                                - (self._row_set(r)
                                   | self._column_set(c)
                                   | self._subsquare_set(r, c))) == 0:
                    # if the item is empty and row, column, subsquare already
                    # has all required return True
                    return True
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        solver = DfsSolver()
        # solve the puzzle
        lst = solver.solve(self)
        # check if the puzzle has a solution
        if len(lst) == 0:
            # if not return False
            return False
        # if the puzzle has a solution, check if we can find a solution that
        # is different from the previous one
        elif len(solver.solve(self, {str(lst[-1])})) != 0:
            # if there is any other way of finding the solution without
            # including the previous one, return False
            return False
        elif len(solver.solve(self, {str(lst[-1])})) == 0:
            # if no other solutions can be found return True
            return True
        return False
